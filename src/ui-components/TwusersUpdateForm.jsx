...
export default function TwusersUpdateForm(props) {
  const {
    id,
    twusers,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...twusersRecord };
    setUser(cleanValues.user);
    setLetter(cleanValues.letter);
    setNid(cleanValues.nid);
    setErrors({});
  };
  const [twusersRecord, setTwusersRecord] = React.useState(twusers);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Twusers, id) : twusers;
      setTwusersRecord(record);
    };
    queryData();
  }, [id, twusers]);
  React.useEffect(resetStateValues, [twusersRecord]);
  const validations = {
    user: [],
    letter: [],
    nid: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Twusers.copyOf(twusersRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "TwusersUpdateForm")}
    >
      <TextField
        label="User"
        isRequired={false}
