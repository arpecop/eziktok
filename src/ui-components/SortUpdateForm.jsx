...
export default function SortUpdateForm(props) {
  const {
    id,
    sort,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...sortRecord };
    setType(cleanValues.type);
    setUid(cleanValues.uid);
    setTitle(cleanValues.title);
    setDescription(cleanValues.description);
    setImages(cleanValues.images);
    setOneone(cleanValues.oneone);
    setSortOneoneId(cleanValues.sortOneoneId);
    setErrors({});
  };
  const [sortRecord, setSortRecord] = React.useState(sort);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Sort, id) : sort;
      setSortRecord(record);
    };
    queryData();
  }, [id, sort]);
  React.useEffect(resetStateValues, [sortRecord]);
  const validations = {
    type: [{ type: "Required" }],
    uid: [],
    title: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Sort.copyOf(sortRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "SortUpdateForm")}
    >
      <TextField
        label="Type"
        isRequired={true}
